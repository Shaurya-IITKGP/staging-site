...
const Cardss: React.FC<CardssProps> = ({ overlay, setOverlay, setinn }) => {
  ...
  return (
    ...
    <div className='grid justify-center m-16 grid-flow-row grid-cols-3 gap-20 mt-48'>
     <div>
      ...
      <div className='-mt-36'>
        <ul className='grid'>
          {sports.slice(0,5).map((item, index) => (
            <li key={index}><div className={`relative gap-20 inline-grid`}><Card name={sports[index+4]} des={desc[index+4]} prize={prize[index+4]} overlay={overlay} setOverlay={setOverlay} inn={index} setinn={setinn}/><div/></div></li>
          ))}
        </ul>
      </div>
      <div>
      <ul className='grid'>
        {sports.slice(0,4).map((item, index) => (
          <li key={index}><div className={`relative gap-20 inline-grid`}><Card name={sports[index+9]} des={desc[index+9]} prize={prize[index+9]} overlay={overlay} setOverlay={setOverlay} inn={index} setinn={setinn}/><div/></div></li>
        ))}
      </ul> 
      </div>
